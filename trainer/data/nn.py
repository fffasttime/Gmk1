...
class TFProcess:
    ...
    """
    def process(self, batch, testset = None):
        ...
        if steps % 1000 == 0 and (not testset is None):
            policy_loss, mse_loss = self.session.run(
            [self.policy_loss, self.mse_loss],
            feed_dict={self.training: False,
                       self.x:testset[0], self.y_:testset[1], self.z_: testset[2]})

            test_summaries = tf.Summary(value=[
                tf.Summary.Value(tag="Policy Loss", simple_value=policy_loss),
                tf.Summary.Value(tag="MSE Loss", simple_value=mse_loss)])
            self.test_writer.add_summary(test_summaries, steps)
            
            print("step {}, training accuracy={:g}, mse={:g}".format(
                steps, policy_loss, mse_loss))
        ...
        with self.graph.as_default():
            for e, weights in enumerate(self.weights):
                ...
                if isinstance(weights, str):
                    ...
                    new_weight = tf.transpose(tf.constant(new_weights[e], shape=shape), [2, 3, 1, 0])
    ...
    def init_training(self, parafile, trainingfile):
        ...
        loss = 1.0 * self.policy_loss + 0.25 * self.mse_loss + reg_term

        opt_op = tf.train.MomentumOptimizer(
            learning_rate=settings.LR, momentum=0.9, use_nesterov=True)
    ...
    def construct_net(self, planes):
        # Network structure
        RESIDUAL_FILTERS = settings.FLITERS
        RESIDUAL_BLOCKS = settings.BLOCKS
